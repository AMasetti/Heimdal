...
def UPPRecieve(s,verbose=0):
	...
	for i in range(datasntchnl*2+6,datasntchnl*4+6,2):
		Rigth[r] =  int(''.join(str(ord(c)) for c in data[i:i+1]))
		r=r+1
	if verbose==1:
...
def PrintClassic():
	Logo=['______ _____ ______ _____                     _    _ _   _ _____ ','|  ____/ ____|  ____|_   _|   /\              | |  | | \ | |  __ \ ']
	
	for j in range(0,Logo.shape(1)):
		for i in range (0,len(Logo[j])):
			print L1[0:i]
			os.system('clear')
